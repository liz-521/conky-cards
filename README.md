...
*Lightweight Conky cards for system monitoring & AI token usage on your desktop.*
...
- **可调**：0.7×–2.0× 等比缩放、六档透明度（含“玻璃”档：背景全透、文字实色）、拖动后一键记住位置（两段校准消除 conky 的 gap 字体留白偏差）
...
- **状态告警**：GPU/CPU 温度 ≥85°、内存/根盘 ≥92%、今日 token 达 7 日峰值 100% 时弹通知（10/60 分钟同类冷却）
...
安装内容：配置 → `~/.config/conky-ai-cards/`，命令 → `~/.local/bin/`，应用菜单“悬浮卡”+ 开机自启。
